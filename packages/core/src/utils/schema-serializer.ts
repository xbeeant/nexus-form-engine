...
/**
 * Schema Serializer - Schema序列化/反序列化
 *
 * 提供Schema的压缩、序列化、反序列化、diff等功能
 */
export class SchemaSerializer {
  ...
  static serialize(
    ...
    const serialized = SchemaSerializer.serializeSchema(schema, opts);

    if (opts.compress) {
      return SchemaSerializer.compress(JSON.stringify(serialized));
    }
  ...
  static diff(oldSchema: NexusSchema, newSchema: NexusSchema): DiffResult {
    ...
    for (const path of allPaths) {
      ...
      if (!oldValue && newValue) {
        ...
        // 检查是否变化
        const diff = SchemaSerializer.diffSchema(oldValue, newValue);
        if (diff.length > 0) {
   ...
   * @returns 序列化后的对象
   */
  private static serializeSchema(
    schema: NexusSchema,
    ...
    if (schema.properties) {
      ...
      for (const [key, value] of Object.entries(schema.properties)) {
        serialized.properties[key] = SchemaSerializer.serializeSchemaNode(
          value,
          options,
        );
      }
   ...
   * @returns 序列化后的节点
   */
  private static serializeSchemaNode(
    node: any,
    options: SerializeOptions,
  ): any {
    if (!node) {
    ...
    // 序列化items（数组节点）
    if (node.items) {
      serialized.items = SchemaSerializer.serializeSchemaNode(
        node.items,
        options,
      );
    }
   ...
   */
  static compressToBase64(schema: NexusSchema): string {
   ...
   * @returns NexusSchema
   */
  static decompressFromBase64(base64: string): NexusSchema {
   ...
   * @returns Schema大小（字节）
   */
  static sizeOf(schema: NexusSchema): number {
    const serialized = SchemaSerializer.serialize(schema);
   ...
   * @returns 压缩率（百分比）
   */
  static compressionRate(schema: NexusSchema): number {
